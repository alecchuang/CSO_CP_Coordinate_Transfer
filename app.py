...
st.markdown("**(Probe / Fuse / AOI / ATK)**")
st.divider()
...
with col1:
    st.markdown("### X 座標輸入")
    probe_x_input = st.text_input("Probe X:")
    fuse_x_input = st.text_input("Fuse X:")
    aoi_x_input = st.text_input("AOI X:")
    atk_x_input = st.text_input("ATK X:", help="支援的字元: 0-9, A, C-H, J-N, P-S")

with col2:
    st.markdown("### Y 座標輸入")
    probe_y_input = st.text_input("Probe Y:")
    fuse_y_input = st.text_input("Fuse Y:")
    aoi_y_input = st.text_input("AOI Y:")
...
if st.button("Start 轉換", type="primary", use_container_width=True):
    ...
    if "ERROR" in (probe_x, probe_y, fuse_x, fuse_y, aoi_x, aoi_y):
        st.error("[錯誤] Probe / Fuse / AOI 請確保輸入有效的數字！")
        st.stop()
    ...
    if probe_x is not None and probe_y is not None:
        ...
        log_messages.append(f"[Input Probe XY] -> Fuse [X,Y]: [ {out_fuse_x} , {out_fuse_y} ]") 
        log_messages.append(f"[Input Probe XY] -> AOI  [X,Y]: [ {out_aoi_x} , {out_aoi_y} ]") 
        log_messages.append(f"[Input Probe XY] -> ATK  [X,Y]: [ {out_atk_x_char} , {out_atk_y_char} ]") 
    elif probe_x is not None or probe_y is not None:
        log_messages.append("[Input Probe XY] Fail：Need input Probe X and Probe Y together。")
    else:
        log_messages.append("[Input Probe XY] No input，Skip。")
    ...
    if fuse_x is not None and fuse_y is not None:
        ...
        log_messages.append(f"[Input Fuse XY]  -> Probe[X,Y]: [ {out_probe_x} , {out_probe_y} ]")
        log_messages.append(f"[Input Fuse XY]  -> AOI  [X,Y]: [ {out_aoi_x} , {out_aoi_y} ]")
        log_messages.append(f"[Input Fuse XY]  -> ATK  [X,Y]: [ {out_atk_x_char} , {out_atk_y_char} ]")
    elif fuse_x is not None or fuse_y is not None:
        log_messages.append("[Input Fuse XY] Fail：Need input Fuse X and Fuse Y together。")
    else:
        log_messages.append("[Input Fuse XY] No input，Skip。")
    ...
    if aoi_x is not None and aoi_y is not None:
        ...
        log_messages.append(f"[Input AOI XY]   -> Probe[X,Y]: [ {out_probe_x} , {out_probe_y} ]")
        log_messages.append(f"[Input AOI XY]   -> Fuse [X,Y]: [ {out_fuse_x} , {out_fuse_y} ]")
        log_messages.append(f"[Input AOI XY]   -> ATK  [X,Y]: [ {out_atk_x_char} , {out_atk_y_char} ]")
    elif aoi_x is not None or aoi_y is not None:
        log_messages.append("[Input AOI XY] Fail：Need input AOI X and AOI Y together。")
    else:
        log_messages.append("[Input AOI XY] No input，Skip。")
    ...
    if atk_x is not None and atk_y is not None:
        ...
        log_messages.append(f"[Input ATK XY]   -> Probe[X,Y]: [ {out_probe_x} , {out_probe_y} ]")
        log_messages.append(f"[Input ATK XY]   -> Fuse [X,Y]: [ {out_fuse_x} , {out_fuse_y} ]")
        log_messages.append(f"[Input ATK XY]   -> AOI  [X,Y]: [ {out_aoi_x} , {out_aoi_y} ]")
